...
'''
    ...
    Batch Sizes: The number of samples that will be propogated through the network
        - For instance, if we have 1200 samples, we can set Batch Size to 100 for the algorithim to 
        use samples 1-100, then 101-200, 201-300, etc..
        - This is better than using all of the samples as it requires less memory, is quicker (we update 
        the weights after every propogation)
        - Smaller batches lead to less accurate estimates of the gradient 
    Epoch: One forward pass and one backward pass of all training examples
    Learning Rate: The amount that the weighst are updated during training (between 0 and 1.0)
    ENum: set of symbolic names (members) bound to unique, constant values. Within an enumeration, 
